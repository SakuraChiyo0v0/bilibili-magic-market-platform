...
### 1.3 个人中心
- [x] **个人资料**：修改昵称、头像、修改密码。
- [x] **API Key 管理**：(高级功能) 为高级用户提供 API Key，允许他们通过代码访问数据。
...
## 阶段六：性能优化与移动端 (Performance & Mobile)
**目标**：提升系统响应速度，优化移动端体验，防止数据库膨胀。

### 6.1 缓存层 (Redis)
- [ ] **接口缓存**：引入 Redis，对 `/api/items` (列表) 和 `/api/stats` (统计) 进行短时缓存 (e.g., 10秒)，大幅降低数据库压力。
- [ ] **限流优化**：将目前的内存限流 (`InMemoryRateLimiter`) 迁移至 Redis，支持多实例部署。

### 6.2 移动端适配 (Responsive UI)
- [ ] **卡片视图**：在手机端隐藏复杂的 Table 表格，自动切换为“商品卡片”流式布局。
- [ ] **底部导航**：在手机端隐藏侧边栏，改为底部 TabBar 导航。

### 6.3 数据治理 (Data Governance)
- [ ] **历史数据清洗**：新增定时任务，自动清理 3 个月前的 `price_history` 分时数据，仅保留“每日最低价”快照，防止数据库体积爆炸。
- [ ] **数据库自动备份**：集成 `mysqldump` 定时任务，将数据库备份上传至云存储或本地保留。

---

## 阶段七：通知渠道扩展 (Advanced Notifications)
**目标**：提供比邮件更即时、更便捷的触达方式。

### 7.1 IM 机器人 Webhook
- [ ] **多平台支持**：支持配置 钉钉、飞书、企业微信 的 Webhook URL。
- [ ] **模板消息**：发送包含商品图片、价格趋势图的富文本卡片消息。

### 7.2 浏览器通知
- [ ] **Web Push**：利用 Service Worker 实现浏览器原生推送，无需打开网页也能收到降价弹窗。
