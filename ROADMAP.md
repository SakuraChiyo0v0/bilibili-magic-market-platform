...
### 1.1 用户认证系统 (Auth)
- [ ] **注册/登录**：支持用户名/密码注册，未来可扩展邮箱/手机号验证。
- [ ] **JWT 鉴权**：使用 OAuth2 + JWT (JSON Web Tokens) 进行无状态身份验证。
- [ ] **密码加密**：使用 bcrypt 或 Argon2 对用户密码进行哈希存储。

### 1.2 角色与权限 (RBAC)
- [ ] **角色划分**：
    - `Admin`：拥有所有权限，可配置爬虫参数、管理所有用户、查看系统日志。
    - `User`：仅能查看商品、管理自己的关注列表、设置自己的推送通知。
- [ ] **接口保护**：后端 API 增加 `Depends(get_current_active_user)` 依赖注入，区分公开接口与私有接口。
...
- [ ] **我的关注**：用户可以收藏感兴趣的商品 (Many-to-Many 关系)。
- [ ] **目标价格**：收藏时可设置“心理价位”。
- [ ] **专属看板**：Dashboard 增加“我的关注”板块，只看自己关心的商品走势。
